...
All configuration options are specified in your `application.yml` or `application.groovy` file under the `grails.plugins.server-timing` namespace.
...
| `grails.plugins.server-timing.enabled`
| `Boolean`
| `null` (auto-detect)
...
| `grails.plugins.server-timing.metric-key`
| `String`
| `GrailsServerTiming`
