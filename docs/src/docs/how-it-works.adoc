...
1. **ServerTimingAutoConfiguration** - A Spring Boot `@AutoConfiguration` that registers `ServerTimingProperties` and conditionally registers the filter and its `FilterRegistrationBean` when the plugin is enabled (via `@ConditionalOnProperty`)
